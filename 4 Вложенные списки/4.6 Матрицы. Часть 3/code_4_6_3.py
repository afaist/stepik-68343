...
которая создает матрицу размером n×m и заполняет её числами от 1 до nxm в
...
# в задании указано создать матрицу и заполнить ее
matrix = [[i * m + j for j in range(1, m + 1)] for i in range(n)]
# вывод матрицы в соответствии с заданием
for i in range(n):
    for j in range(m):
